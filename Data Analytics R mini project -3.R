...
knn_classifier <- function(x_train, y_train, x_test, distance_method, k){
  ...
  dista<-calculate_distance_matrix(x_train, x_test, distance_method)
  if(distance_method=='calculate_euclidean')
  {
      met=FALSE #ascending order
  }
  else
  {
      met=TRUE #descending order
  }
  for(row in 1:nrow(dista))
  {
      ds<-sort(dista[row,],index.return=TRUE, decreasing=met)$ix
      ds<-ds[1:k] #returns indeces of k smallest/largest values
      lab<-c(0,0,0,0)
      for(i in 1:k)
      {
          lab[y_train[ds[i]]]<-lab[y_train[ds[i]]]+1
      }
...
knn_classifier_confidence <- function(x_train, y_train, x_test, distance_method='calculate_cosine', k){
  ...
  for(row in 1:nrow(dista))
  {
      ds<-sort(dista[row,],index.return=TRUE, decreasing=TRUE)$ix
      ds<-ds[1:k] #returns indeces of k smallest/largest values
      lab<-c(0,0,0,0)
      for(i in 1:k)
      {
          lab[y_train[ds[i]]]<-lab[y_train[ds[i]]]+dista[row,ds[i]]
      }
      d<-(which.max(lab))
      ans=c(ans,d)
...
dtree <- function(x_train, y_train, x_test){
  ...
  df<-cbind(x_train_df,y_train_df)
  fit<-rpart(y_train~.,data=df,parms=list(split="gini"))
  predicted<-predict(fit,newdata=x_test,type="class")
  return (predicted)
...
dtree_cv <- function(x_train, y_train, x_test, n_folds){
  ...
  # HINT1: Make sure to read the documentation for the caret package. Check out the 'train' and 'trainControl' functions.
  fit_cv<-train(y_train~.,data=df,method="rpart",trControl=trctrl,parms=list(split="gini"),tuneLength=100)
  predicted<-predict(fit_cv,newdata=x_test)
  return(predicted)
...
calculate_accuracy <- function(y_pred, y_true){
  ...
  cm<-confusionMatrix(y_pred,y_true,dnn=c("Prediction","Reference"))
  overall.accuracy <- cm$overall['Accuracy']
  return(list(as.table(cm),overall.accuracy))
